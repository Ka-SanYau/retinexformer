...
class Illumination_Estimator(nn.Module):
    def __init__(
            self, n_fea_middle, concat_poe_map=False, n_fea_in=4, n_fea_out=3, add_meance=False):  #__init__部分是内部属性，而forward的输入才是外部输入
        super(Illumination_Estimator, self).__init__()

        self.conv1 = nn.Conv2d(n_fea_in, n_fea_middle, kernel_size=1, bias=True)

        self.n_feat_mid = n_fea_middle
        self.concat_poe_map = concat_poe_map
        self.add_meance = add_meance

        self.depth_conv = nn.Conv2d(
            n_fea_middle, n_fea_middle, kernel_size=5, padding=2, bias=True, groups=n_fea_in)
        
        self.leakRelu = nn.LeakyReLU(0.01, inplace=True)
        self.down_fea1 = nn.Conv2d(n_fea_middle * 2, n_fea_middle, 3, 1, 1, bias=True)
        self.down_fea2 = nn.Conv2d(n_fea_middle + 48, n_fea_middle, 3, 1, 1, bias=True)
    ...
    def forward(self, img):
        ...
        if self.concat_poe_map:
            ...
            color_md_1 = get_poe_new2(b, h, w, tmp, 0, 16)
            color_md_2 = get_poe_new2(b, h, w, tmp, 1, 16)
            color_md_3 = get_poe_new2(b, h, w, tmp, 2, 16)
            ce = torch.cat([color_md_1, color_md_2, color_md_3], dim=1)
            

        # stx()
        input = torch.cat([img,mean_c], dim=1)
        x_1 = self.conv1(input)
        # x_1 = self.down_fea1(torch.cat([x_1, img_mean_ce], dim=1)) # this is down_fea 1
        if self.add_meance:
            img_mean_ce = get_poe_new2(b, h, w, mean_c, 0, 48)
            # x_1 = self.down_fea1(torch.cat([x_1, img_mean_ce], dim=1)) # this is down_fea 1
            ce += img_mean_ce
            if flag:
                print("add mean ce...")
        ...
        if self.concat_poe_map:
            x_1 = self.down_fea2(torch.cat([x_1, ce], dim=1)) # this is down_fea 1
            if flag:
                print("add ce down fea...")
        # else:
        #     img_mean_ce = get_poe_new2(b, h, w, mean_c, 0, self.n_feat_mid)
        #     x_1 = self.down_fea1(torch.cat([x_1, img_mean_ce], dim=1)) # this is down_fea 1
...
class RetinexFormer_Single_Stage(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, level=2, num_blocks=[1, 1, 1], cat_poe=False, add_meance=False):
        super(RetinexFormer_Single_Stage, self).__init__()
        self.estimator = Illumination_Estimator(n_feat, concat_poe_map=cat_poe, add_meance=add_meance)
